...
jsonpath=r"\\acdev01\3M_CAC\ITSMWorklogUpdate\itsmworklog.json"
...
class monitorMail():
    def _connect(self):  # (self, aeuser, aepassword, aeci, aeparameters):
        ...
        '''
        o365mailid=data_.get("o365mailid","").strip()#"ussacdev@mmm.com"
        mailusername=data_.get("mailusername","").strip()#data_["user1"].strip()
        proxy = {}
        proxy.update({})  # (aeci.get("proxy", {}))
        ProxyAdapter.proxies = proxy
        BaseProtocol.HTTP_ADAPTER_CLS = ProxyAdapter
        smtp_address =data_.get("smtp_address","").strip() #"ussacdev@mmm.com"  # "mailserv.mmm.com"#aeci.get("primarysmtpaddress", aeuser)
        if not smtp_address:
            smtp_address =data_.get("smtp_address","").strip()  # aeuser
        server =data_.get("server","").strip() #"Outlook.Office365.com"  # "mailserv.mmm.com"# aeci.get("server", "Outlook.Office365.com")
        if not server:
            server =data_.get("server","").strip()
...
def connectFolder():
    ...
    global mailCollections
    folder_name =data_.get("folder_name","").strip() #"ITSMworklogupdate"  # "ApplensSMO"#aeparameters.get("folder_name")
    if folder_name:
...
username =data_.get("username","").strip()#data_["user2"].strip()#"USSACITSMDev"
password =os.popen(passwordUpdate.format(userid=username)).read().strip() #"Ts)Q5Ll3N-ICF_pa9hcl"  # "wmkJjlrO0Ew>cObVpP_<"
host = data_.get("host","").strip()#"http://itsmwsqa.mmm.com/"  # "http://itsmws.mmm.com/"
table = data_.get("table","").strip()#"AC_IncidentManagement"
...
def postconnect():
    ...
    mailitems=connectFolder()
    #print(mailitems)
    #for maildetails in range(0, len(mailCollections)):
    if mailitems!= None:
        for maildetails in range(0, len(mailitems)):
            mailsubject = mailitems[list(mailitems.keys())[maildetails]]["subject"].strip()
            incidentID = mailsubject[mailsubject.index("ID:") + len("ID:"):mailsubject.index("CODE:")].strip()
            mailbody = mailitems[list(mailitems.keys())[maildetails]]["body"].strip()
            From=mailitems[list(mailitems.keys())[maildetails]]["mailaddress"].strip()
            ...
            ticketstatus=data_["ticketStatus"][mailsubject[mailsubject.index("O365AUTO(")+len("O365AUTO("):mailsubject.index("O365AUTO(")+len("O365AUTO(")+2].strip().replace(")","")]
            print(ticketstatus)
            ID = incidentID
